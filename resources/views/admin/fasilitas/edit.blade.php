...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 bg-slate-50 border-b border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-1">Perbarui sarana prasarana pesantren.</p>
        </div>

        <!-- Form Content -->
        <form action="{{ route('admin.fasilitas.update', $fasilitas->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <!-- Upload Foto -->
            <div class="space-y-4">
                <label class="block text-sm font-bold text-slate-700">Foto Fasilitas</label>
                
                @if ($fasilitas->path_foto)
                    <div class="flex items-start space-x-4 p-4 bg-slate-50 border border-slate-200 rounded-lg">
                        <img src="{{ asset('storage/' . $fasilitas->path_foto) }}" alt="{{ $fasilitas->nama_fasilitas }}" class="w-32 h-32 object-cover rounded-lg shadow-sm border border-slate-200">
                        <div>
